...
def load_and_preprocess_data_drom_csv(name_file="/home/yagor/Рабочий стол/mipt/lab3/notebook/data_from_hbase_with_postprocess.csv"):
    """
            Function which load and preprocess data for train Kmeans
        Args:
...
def train_model(cluster_final_data, k=8):
    """
            Function which train and predict clusters with KMeans model
        Args:
...
def save_results(result, name_file = "output_result.csv"):
    """
            Function which save predict clusters with indexs
        Args:
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder \
      ...
      .appName("SparkByExamples.com") \
      .getOrCreate()
    cluster_final_data = load_and_preprocess_data_drom_csv(name_file)
    result = train_model(cluster_final_data, k)
    save_results(result, name_file = file_for_save_result)
