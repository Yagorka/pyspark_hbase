...
class Utils():
    """
        Сlass that allows you load train and predict data from hbase to .csv
    """
    @staticmethod
    def load_data_from_hbase(name_tamle):
        """
                Class method which scan and load data from HBase
            Args:
                name_tamle str: name table from Hbase on localhost
            Returns:
                dict[dict]: all data from table
        """
    ...
    def postprocess_train_data_from_hbase(all_data):
        """
                Class method which decode and postprocess train data
            Args:
    ...
    def postprocess_result_data_from_hbase(all_data):
        """
                Class method which decode and postprocess result data
            Args:
                all_data dict(): dict with dicts rows
            Returns:
                list[dict]: decode data from all_data
    ...
    def save_data_from_list_keys(decode_all_data, name_csv_for_save : str ='data_from_hbase.csv'):
        """
                Class method which saves decode and postprocess result data
            Args:
...
if __name__ == "__main__":
    ...
    all_data = Utils.load_data_from_hbase(name_table)
    if train_data:
        decode_all_data = Utils.postprocess_train_data_from_hbase(all_data)
    else:
        decode_all_data = Utils.postprocess_result_data_from_hbase(all_data)
    Utils.save_data_from_list_keys(decode_all_data, file_for_saves)
